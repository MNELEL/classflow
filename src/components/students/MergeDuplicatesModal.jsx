...
function normName(n) {
  return (n || '').trim().toLowerCase().replace(/[^א-תa-z0-9]/g, '');
}

const ID_KEYS = ['id_number'];

// Group duplicate students by normalized name or shared student ID number.
function findGroups(students) {
  ...
  ID_KEYS.forEach(key => {
    ...
    students.forEach((s, i) => {
      ...
      if (v && String(v).trim()) { const k = String(v).trim(); (byId[k] ||= []).push(i); }
    });
...
function fieldCount(s) {
  return s.custom_fields ? Object.keys(s.custom_fields).filter(k => s.custom_fields[k]).length : 0;
}
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="max-w-[95vw] sm:max-w-md" dir="rtl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Merge className="w-5 h-5 text-amber-600" /> מיזוג כפילויות תלמידים</DialogTitle>
        </DialogHeader>
        {activeGroups.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-6">לא זוהו כפילויות בין התלמידים הקיימים.</p>
        ) : (
          <div className="space-y-3 max-h-[70vh] overflow-y-auto">
            <div className="flex items-start gap-2 text-xs text-amber-700 dark:text-amber-400 bg-amber-50/60 dark:bg-amber-900/15 rounded-lg p-2.5">
              ...
              <span>זוהו {activeGroups.length} קבוצות כפולות (לפי שם או תעודת זהות). בחר איזה תלמיד לשמור — כל הנתונים (ציונים, נוכחות, פרטים אישיים, חברים וכו') ימוזגו אליו והשאר יימחקו.</span>
            </div>
