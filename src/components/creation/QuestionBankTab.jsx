...
export default function QuestionBankTab({ onGoToExam }) {
      ...
      <div className="px-4 pt-4 pb-3 border-b border-border bg-card">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-base">בנק שאלות</h1>
              <p className="text-xs text-muted-foreground">שאלות → דף עבודה להדפסה</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {selected.length > 0 && (
            ...
            <button onClick={onGoToExam} className="text-[11px] px-3 py-1.5 rounded-lg bg-primary text-primary-foreground flex items-center gap-1.5 hover:bg-primary/90 transition-colors">
              <FileText className="w-3.5 h-3.5" /> הרכבת מבחן
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {allQuestions.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm mt-1">צור דפי עבודה תחילה בלשונית "יצירה חדשה"</p>
            </div>
          ) : filtered.length === 0 ? (
