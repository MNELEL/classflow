...
import { Sparkles, Printer, Save, Star, StarOff, Trash2, BookOpen, ChevronUp, ChevronDown } from 'lucide-react';
            ...
            <Card key={ws.id} className="border-border/60">
              ...
              >
                ...
                <div className="flex gap-1 shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); setGenerated(ws); toast.success('נטען לעריכה'); }} aria-label="טען לעריכה" className="text-primary hover:text-primary/70">
...
export default function CreateTab() {
  const [sub, setSub] = useState('generators');
  return (
    <div className="h-full overflow-y-auto" dir="rtl">
      <div className="px-4 pt-3 pb-2 sticky top-0 bg-background z-10 border-b border-border/60">
        <div className="flex gap-1 bg-muted rounded-lg p-1 max-w-md mx-auto">
          <button onClick={() => setSub('generators')} className={`flex-1 py-1.5 rounded text-xs font-medium transition-colors ${sub === 'generators' ? 'bg-card shadow text-foreground' : 'text-muted-foreground'}`}>
            ✨ סיכומים ומשימות
          </button>
          <button onClick={() => setSub('worksheet')} className={`flex-1 py-1.5 rounded text-xs font-medium transition-colors ${sub === 'worksheet' ? 'bg-card shadow text-foreground' : 'text-muted-foreground'}`}>
            📝 דף עבודה מ-AI
          </button>
        </div>
      </div>
