...
import { Mail, FileDown, Loader2, RotateCcw } from 'lucide-react';
...
} from '@/lib/weeklySheet';

export default function WeeklySheetTab() {
  ...
  const [draft, setDraft] = useState(() => makeDefaultWeeklySheet({}));
  ...
  const setSubject = (index, content) => {
    update({ subjects: draft.subjects.map((s, i) => (i === index ? { ...s, content } : s)) });
  };

  async function handleExport() {
    setExporting(true);
    try {
      const { exportWeeklySheetPdf } = await import('@/lib/weeklySheetExport');
    ...
    <div className="space-y-4">
      <div>
        <h2 className="flex items-center gap-2 text-lg font-bold">
          <Mail className="w-5 h-5 text-primary" aria-hidden="true" /> דף קשר שבועי להורים
        </h2>
        <p className="text-sm text-muted-foreground mt-1">
          מלא את הספק החומר, המבחנים וההודעות של השבוע — והמערכת מפיקה דף קשר מוכן להדפסה:
          שער, הספק ודף חתימת הורים. הפרטים נשמרים אצלך לשבוע הבא.
        </p>
      </div>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm">כיתה ושבוע</CardTitle>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm">הספק החומר לפי מקצועות</CardTitle>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm">מבחנים, הודעות ויישר כח</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-1">
